...
(function () {
  ...
  const SFX = {
    ...
    /** 鹤仙瞬移：高频气声上滑 + 闪音（呼啸消散） */
    craneTele() {
      noiseAt(0, 0.22, 0.07, 'bandpass', 2200, 5200);
      tone(1760, 0.16, 'sine', 0.08, 2640);
      tone(880, 0.18, 'triangle', 0.05, 1320, 0.06);
    },
    /** 鹤仙风炮/风刃：气流短促喷射（噪声带通 + 三角波下扫） */
    craneWind() {
      noiseAt(0, 0.14, 0.06, 'bandpass', 1400, 3600);
      tone(1040, 0.12, 'triangle', 0.06, 520);
    },
    /** 鹤仙龙卷风：低频呼啸上行 + 持续气旋呼啸 */
    craneTornado() {
      tone(140, 0.7, 'sawtooth', 0.08, 320);
      noiseAt(0.05, 0.7, 0.11, 'bandpass', 360, 1400);
    },
    /** 鹤仙镜面：清脆水晶合鸣（高正弦双音 + 泛音） */
    craneMirror() {
      tone(1568, 0.3, 'sine', 0.08, null);
      tone(2093, 0.4, 'sine', 0.06, null, 0.05);
      tone(2637, 0.25, 'sine', 0.04, null, 0.12);
    },
    /** 鹤仙反射激光：充能啸叫 + 击发嗡鸣 */
    craneLaser() {
      tone(880, 0.35, 'sawtooth', 0.08, 1320);
      noiseAt(0.2, 0.3, 0.06, 'bandpass', 900, 2400);
    },
